...
func TestRedact_AWSKeys(t *testing.T) {
	in := []byte("id1=@@SEC_AWS_AKID@@ id2=@@SEC_AWS_ASIA@@ done")
...
func TestRedact_AnthropicBeforeGeneric(t *testing.T) {
	in := []byte("key @@SEC_ANTHROPIC_AB@@ here")
...
func TestRedact_DotenvGatedByKeyName(t *testing.T) {
	...
	require.NotContains(t, string(out), "@@SEC_STRIPE@@")
}
...
var secretAlphabet = []string{
	"@@SEC_AWS_AKID@@", "@@SEC_ANTHROPIC_AB@@", "password=", "hunter22",
...
var literalProbes = map[string]string{
	...
	"aws/AKIA": "id=@@SEC_AWS_AKID@@",
	"aws/ASIA": "id=@@SEC_AWS_ASIA@@",
...
const pemProbe = "@@SEC_PEM_RSA_BEGIN@@\nMIIBOgIBAAJBAKj34GkxFhD9\n@@SEC_PEM_RSA_END@@"
...
var prefilterProbes = []string{
	...
	"PRIVATE KEY-----", "private key-----", "@@SEC_PEM_RSA_BEGIN@@",
	"AKIA", "ASIA", "akiaiosfodnn7example", "@@SEC_AWS_AKID@@",
	"ghp_", "GHP_1234567890abcdefghijklmnopqrstuvwxyz12", "github_pat_", "ghz_",
	"sk-ant-", "SK-ANT-", "sk-", "sk_", "@@SEC_ANTHROPIC_AB@@",
	"eyJ", "EYJ", "@@SEC_JWT@@",
...
func TestRedactorSuite_BehaviourIsNotSkipped(t *testing.T) {
	_, ms := redact.New(config.Defaults()).Redact([]byte("aws_access_key_id = @@SEC_AWS_AKID@@"))
