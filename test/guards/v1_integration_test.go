...
func v1RoundTrip(t *testing.T, key, path string) {
	...
	case strings.HasSuffix(path, ".jsonc"), strings.HasSuffix(path, ".jsonl"), strings.HasSuffix(path, ".json"):
...
// v1RoundTripJSON is the arm for every JSON-flavoured frozen artifact: .json, .jsonl and .jsonc. It
// unmarshals into the declared Go type and asserts the type can reproduce the bytes exactly.
