...
> **⑧ first.** It is the only item here that can be destroyed by the merge it describes, and the only one whose evidence expires. If you read nothing else in this section, run its two greps.
...
**⑧ THIS FILE WAS EDITED ON TWO WAVE-1 BRANCHES AT ONCE. Verify both halves survived the merge before running anything below.** SP-06 added §2.6a (this section). SP-04 independently added a recommended-model header and a **new §2.0 merge-inspection section with `V2-MERGE-*` rows**, which did not exist on `develop`. The two touch different regions, so git merges them **cleanly and silently** — which is the danger: nothing will conflict, and nothing will announce that a half went missing if a resolution took one side wholesale. Run both greps against the merged file and require both:

```bash
F=plans/V2-VERIFY-primitives-store-dag-and-baseline.md
grep -c  '^#### 2.6a'      "$F"   # SP-06's half → must be 1
grep -cE 'V2-MERGE-[0-9]'  "$F"   # SP-04's half → must be > 0
grep -c  '^> \*\*Recommended model' "$F"  # SP-04's half, 2nd witness → must be 1
```

Those patterns are deliberately **self-exclusive**: they match the section heading and the row IDs, not the prose of this warning, which names both halves and would otherwise satisfy its own check. (Written the obvious way first — `grep -c "2.6a Carried forward from SP-06"` and `grep -c "V2-MERGE-"` — both returned a match on a file missing SP-04's half entirely, reporting success for exactly the state they exist to catch. Do not "simplify" them back.)

If either is zero the merge lost content, and the missing half is **gates this checkpoint is supposed to run** — §2.0's own note says three of its rows read state that §5 and §7 later overwrite, so once the fan-out has started those questions can no longer be answered at all. Recover with `git show <branch>:plans/V2-VERIFY-primitives-store-dag-and-baseline.md` and re-apply. The correct resolution is a **union of both edits**; never `-X ours` / `-X theirs` on a plan file.

**One caveat that makes this urgent rather than routine:** when SP-06 landed, SP-04's half was **uncommitted in its worktree** (`C:\Users\Quant\Documents\Programming\Projects\qompack`, branch `feat/sp04-…`, 65 insertions). If it was never committed there, no branch carries it and `git show` cannot recover it — check that worktree's `git status` and `git stash list` **before** any `checkout`, `reset` or `stash drop` touches it. That is the one loss mode with no undo.
