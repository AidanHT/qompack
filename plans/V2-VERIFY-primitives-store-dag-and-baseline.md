...
#   9390976 feat(sp07)   148c8aa feat(sp06)   9e054de feat(sp02)
#   532ffbe feat(sp04)   c441a67 feat(sp03)   ecea37a feat(sp05)
...
| V2-MERGE-09 | **`//nomagic:allow` inventory matches what the subplans authorized.** Each subplan permits a specific, named set; nothing enumerates the union | `grep -rn "nomagic:allow" --include=*.go .` | Every annotation is one a subplan explicitly declared, with its stated reason. SP-03 authorizes exactly two (`FPWarnRate` in `internal/sketch/bloom.go`, and `mhJaccardTolerance` in `internal/sketch/sketchtest/minhash.go`). Any annotation not traceable to a subplan's Implementation spec was added to silence a lint failure rather than to record a decision, and is a failure of `00-ARCHITECTURE.md` §11.6 regardless of the build being green |
...
| V2-MERGE-17 | **`internal/cli/commands.go` is edited by two branches within two lines of each other.** SP-02 appends `evalCmds()` to `All()`; SP-05 adds `daemon` and `self-test` as real `Cmd` literals and deletes their two rows from `notImplemented` | `go run ./cmd/qompack --help` ; `go test -run 'TestSelfTest_RegisteredInAll\|TestDispatch_' ./internal/cli/` ; `grep -n 'notImplemented' -A 20 internal/cli/commands.go` | `eval import`, `daemon` and `self-test` are all registered and runnable; **neither `daemon` nor `self-test` remains in `notImplemented`**. The bare `eval` row deliberately stays in `notImplemented`: running the replay harness from the binary is SP-14's `/qompack:eval`, and `internal/cli/register_eval.go` records that ruling — the replay-gate driver is reached via `devtool replay`, never the CLI. A resolution that took one side leaves either the replay-gate driver unreachable from the CLI (breaking V2-SP02-15) or the daemon advertised as unimplemented while it is running (breaking §2.5 outright) |
...
| V2-SP01-13 | `obs`: log-bucket histograms, budget IDs B-A…B-G | `go test ./internal/obs/...` | green, incl. `TestBudgets_AllSixPresentAndConfigDriven` (its name predates B-G; it asserts seven), `TestBudgets_BGCoversTheDegradedSpoolAppend`, `TestHistogram_PercentileConservative`, `TestCheckBudgets_CountsConsecutiveWindows`. SP-01 shipped B-A…B-F; **B-G `hook_degraded` was added by the post-V2 hardening round** (V2-report §16.6 item 24) and is reported only — see V2-SP05-24 |
...
| V2-SP01-18 | Conformance suites still enforce Rule W-1 for **unimplemented** packages | `go test -v -count=1 ./internal/... 2>&1 \| grep -c 'behaviour: implementation is a stub (Rule W-1)'` (**`-v` is load-bearing** — a non-verbose run never prints skip reasons, so the count is 0 and the grep exits 1; V1 M2 fixed this once already) | > 0, and **every** remaining skip in the tree carries that exact message or `contract fixture not yet recorded (Rule W-2)`. See V2-SP01-19 for which packages may still skip. |
| V2-SP01-19 | Rule W-1 skips are gone from every wave-0/1 package | `go run ./tools/devtool lint` (`stubskips` sub-check) ; `grep -rn "t.Skip" internal/sketch internal/chunk internal/canon internal/symbols internal/store internal/redact internal/tokens internal/dag internal/ipc internal/contract internal/eval` | the `stubskips` sub-check is the authoritative gate and must be green. The grep is a classification aid, not a zero-output assertion — it returns ~45 lines, every one of which must classify as a comment, a `<pkg>test/` Rule W-1 skip helper, or a `platform:`-prefixed skip; **no behaviour skip may fire against a real wave-1 implementation** (confirm with `-v`: zero `--- SKIP` on behaviour subtests). The wave-1 `<pkg>test` suites keep their skip machinery and their deliberate `*_StubIsSkipped` self-tests — that is Rule W-1 working, not a violation |
...
| V2-SP01-24 | Pure functions SP-01 implemented rather than stubbed | `go test -run 'TestRootHash_Formula\|TestYoungDaly_Formula\|TestSkiRental_ComputedNotLiteral\|TestDescriptorKey_Stable\|TestStripInjections\|TestTombstone_RendersTheSection81Form' ./internal/...` | green (the tombstone test was misnamed here and in V1 L10 — the real name is `TestTombstone_RendersTheSection81Form`); `grep -R "12\.5" internal/ --include=*.go` outside `_test.go` and doc comments returns nothing — `internal/sketch/doc.go` legitimately quotes the forbidden-literal list, so the executable gate is `nomagic`, not this grep |
| V2-SP01-25 | SP-01 benchmark budgets still met | `go test -bench 'BenchmarkHistogram_Observe\|BenchmarkConfigLoad_ColdNoFiles\|BenchmarkPathsWriteAtomic_4KB' -benchmem ./internal/obs ./internal/config ./internal/paths` | < 100 ns/op, < 2 ms/op, < 2 ms/op respectively. **`PathsWriteAtomic_4KB` misses its 2 ms budget on this host by construction** (§4.6's pre-rename fsync alone costs ~2.1 ms here) — the committed baseline's own header records the miss; the platform-independent halves (`B/op`, `allocs/op`, both pinned by the baseline) are the regression signal (V1 P-4) |
...
**② V2-SP02-17 passed vacuously: `-run TestImports` matched no test.** `go test -run TestImports ./internal/eval/` prints `ok … [no tests to run]` and exits 0. The test is `TestImportGraph_EvalIsFoundationOnly`. The row below now runs `-run TestImportGraph_`. <!-- runpatterns: quotes the unsatisfiable command this finding is about; the corrected row is below -->
...
|---|---|---|
| V2-SP02-13 | "all 19 `TestGate_*` rows" | **17** `TestGate_*`, plus 3 `TestPhase0_*`, plus **15** `TestReplayDriver_*`, plus `TestRunPhaseChecks_RunsEveryMergedPhase`, `TestFirstDiffLine` and `TestCanonicalMetrics_StableAcrossMapOrder` — **38** tests in `test/replay`. SP-02's handoff recorded 36 with 13 `TestReplayDriver_*`, which was true when it was written; the post-v2 `--max-cpu` hardening round added `TestReplayDriver_MaxCPUExceeded` and `TestReplayDriver_BothLimitsHoldOnTheCommittedCorpus` |
...
| V2-SP02-04 | Breakpoint OPT (§5.6), permanently disclaimed | `go test -run TestBreakpoint ./internal/eval/` (covers `TestBreakpointOPT_*` and the three `TestBreakpointPlan_*` brute-force checks the old pattern missed) | green; `Plan.Note == NotPluginActionable` on every path |
...
| V2-SP02-13 | **Replay gate driver** — 2 % rule, sign-off trailer, phase assertions | `go test ./test/replay/... -v` ; cross-check the inventory with `go test -list '.*' ./test/replay/` before reading a count as a regression | green: **38 tests — 17 `TestGate_*`, 3 `TestPhase0_*`, 15 `TestReplayDriver_*`**, plus `TestRunPhaseChecks_RunsEveryMergedPhase`, `TestFirstDiffLine` and `TestCanonicalMetrics_StableAcrossMapOrder` (the plan's "19 `TestGate_*`" is wrong; §2.2a ④, and SP-02's handoff's 36/13 predates the post-v2 `--max-cpu` round, which added **`TestReplayDriver_MaxCPUExceeded`** and **`TestReplayDriver_BothLimitsHoldOnTheCommittedCorpus`**). Incl. `TestGate_TwoPercentBoundaryExclusive`, `TestGate_SignOffAllowsNamedMetricOnly`, `TestGate_BloomFPCeiling`, `TestGate_GrowthInconclusiveFails`, **`TestPhase0_SessionCountFloor` / `TestPhase0_RequiresStock` / `TestPhase0_Reproducibility`** (for the plan's `TestGate_Phase0ExitCriterion`) and **`TestReplayDriver_PhaseChecksMayNotBeDisabledInCI`** (for the plan's `TestGate_…`) |
...
| V2-SP04-08 | `FuzzSplit` | `go test -run=XXX -fuzz '^FuzzSplit$' -fuzztime 120s ./internal/chunk/` (unanchored, `-fuzz` matches `FuzzSplitStream` too and go refuses to run) | zero crashers |
...
| V2-SP04-10 | Canon registry: order, duplicate rejection, strip gating | `go test -run 'TestRegistry_\|TestRun_\|TestKnownClassesCoverConfigDefaults\|TestParseClass_\|TestDefault_ForSelectsPerToolCanonicalizers\|TestDefault_Names' ./internal/canon/` (the last two carry this row's stated observables) | `For("Bash","")` returns `[crlf ansi timestamps durations pids addresses tmpPaths bash testrunner git]` in that exact order, 100 repeats identical; `Names()` has the 14 registered names |
...
| V2-SP04-12 | **Normative canon properties** (§5.6) | `go test -run 'TestEveryCanonicalizer_Idempotent\|TestEveryCanonicalizer_NeverGrows\|TestRestore_RoundTrip\|TestCorpus_StructuralProperties' ./internal/canon/` (the row's original three names never existed — it passed vacuously from the day it was written) | `Run(Run(x)) == Run(x)`; `len(canonical) ≤ len(input)` always; `Restore(canonical, deltas) == input` whenever `KeepDeltas` |
| V2-SP04-13 | The seven generic canonicalizers | `go test -run 'TestCRLF_\|TestANSI_\|TestTimestamps_\|TestDurations_\|TestPIDs_\|TestAddresses_\|TestTmpPaths_' ./internal/canon/` | every table row matches; `pid=7` unchanged (short-match guard); `127.0.0.1:54321` → `127.0.0.1:<p>` (the port read 54123 here — transposed digits; the pinned case is `generic_test.go:215`) |
| V2-SP04-14 | The seven per-tool canonicalizers | `go test -run 'TestBash_\|TestTestRunner_\|TestGrep_\|TestGlob_\|TestFileRead_\|TestWebFetch_\|TestGit_' ./internal/canon/` | every table row matches; `TestBash_ProgressCollapse` and `TestBash_TrailingWhitespace` pin the class-gating contract (the row's original two names never existed) |
...
| V2-SP04-17 | Near-dup decision surface (`canon.Decide`) | `go test -run 'TestDecide_\|TestSignature_OnlyWhenEnabled\|TestOptionsFrom_MapsAppendixC' ./internal/canon/` | all 7 `TestDecide_Table` rows match; the golden **`testdata/golden/canon/dedup-decisions.json`** reproduces; `OptionsFrom` maps Appendix C (`permutations 128`, `nearDupThreshold 0.9`, the six strip classes). **Two corrections, both made in place:** the golden path was written as `testdata/golden/contracts/canon/…`, which has never existed — `testutil.GoldenJSON` resolves to `testdata/golden/<pkg>/<name>` (`internal/testutil/golden.go`), and `internal/canon/dedup_test.go`'s `TestDecide_GoldenFixture` writes the file named here — so the old spelling could not have reproduced anything. And the row's title claimed consumption **by SP-06**, which this command cannot observe: it runs `internal/canon`'s own unit tests only, so the title asserted more than the row gated. `Decide` is verified here as a decision surface in its own right; who calls it is not this row's question |
| V2-SP04-18 | Canon golden corpus | `go test -run TestGoldenCorpus_AllFiles ./internal/canon/` (**no** `-update`) | every one of the 30 `testdata/corpora/toolout/**` files (24 at SP-04's tip; the merge added the 4-file `sp06` group and goldens — §2.0b row 3) reproduces its `testdata/golden/canon/**.canon.txt` byte-for-byte |
...
| V2-SP04-27 | Coverage floors | `go run ./tools/devtool cover` **and** `go test -cover -count=1 ./internal/chunk/ ./internal/canon/ ./internal/symbols/` | `internal/chunk` ≥ **90 %**, `internal/canon` ≥ **90 %**, `internal/symbols` ≥ **75 %**. V2-MERGE-14 has landed (`landedSubplans` = SP-01…SP-07), so `cover` binds for all three; the second command is the independent cross-check, no longer the only live measurement |
...
| V2-SP05-08 | Transport permissions | `go test -run 'TestUnixSocketPermissions\|TestStaleUnixSocketReclaimed' ./internal/ipc/` ; on windows `QOMPACK_TEST_ACL=1 go test -run TestWindowsPipeACLRejectsOtherUser ./internal/ipc/` | socket `0600`, directory `0700`; stale socket reclaimed; SDDL contains the current SID and begins `D:P`. **On Windows the two POSIX tests are build-tagged out and the row is vacuous by construction** — the Windows-arm evidence is `GOOS=linux go build ./... && GOOS=linux go vet ./internal/ipc/` |
| V2-SP05-09 | `ipctest` conformance suite, zero skips | `go test -run 'Suite' ./internal/ipc/... -v` ; `go run ./tools/devtool lint` (`stubskips`) | zero behaviour `--- SKIP`s (the deliberate `*_StubIsSkipped` meta-tests and the platform ACL skip are sanctioned); `stubskips` green. A bare `grep t.Skip` here is unsatisfiable — the `skipIfStub*` helpers ARE the Rule W-1 machinery |
...
| V2-SP05-12 | **WAL ingest — the durability boundary** | `go test -run TestIngest ./internal/daemon/` ; `go test -bench BenchmarkIngestAccept ./internal/daemon/` | WAL holds exact bytes; ring full defers processing without blocking and **without any spool fallback** — the WAL-first write already made the line durable, so the spill was deliberately deleted (ruling #23); `l0.ring_full` counted; **ACK precedes processing**; `BenchmarkIngestAccept` meets **B-B p99 < 2 ms** |
...
| V2-SP05-21 | **Contract monitor reports `ModeFull` on a wave-1 build** | `go test -run 'TestFreshBuildReportsModeFull\|TestDeclaredProducerSetMatchesArchitecture' ./internal/contract/ ./internal/daemon/ -v` (the producer-set test moved to `internal/daemon` with ruling #18's commit swap — over contract alone the second observable never runs) | `ModeFull`; exactly **four** assertions at `OK:true, SevInfo, Observed:"not-yet-implemented"` (`precompact.has_time_to_write`, `precompact.custom_instructions_accepted`, `hook.additional_context_delivered`, `mcp.server_registered`); exactly five always-declared producers |
...
| V2-SP05-23 | The nine assertions, individually | `go test -run 'TestSessionStartFires\|TestSourceCompact\|TestSentinel\|TestAdditionalContextGated\|TestPreCompactTimeoutUnknown\|TestCustomInstructionsProbePhrase\|TestHookPayloadShape\|TestPluginRootUnset\|TestWriteMarker_IsTheOnlyMarkerWriterInContract\|TestPanickingAssertionDoesNotDegrade' ./internal/contract/` ; `go test -run TestMarkerIsWrittenByFlushAndCheckpointOnly ./internal/daemon/` (the daemon-route half lives there — ruling #18) | each behaves as its row in §12.1 specifies; `TestPanickingAssertionDoesNotDegrade` proves an assertion crash never degrades the session |
| V2-SP05-24 | Budget table matches the architecture | `go test -run TestBudgets ./internal/obs/` (`TestBudgets_AllSixPresentAndConfigDriven` — the name predates B-G and now asserts seven — and `TestBudgets_BGCoversTheDegradedSpoolAppend`) | **seven** budgets B-A…B-G in order; the first six carry the §2.4 `Clock` strings verbatim (`hook_controlled`, `l0_ingest`, `l0_process`, `hook_wall`, `checkpoint_finalize`, `mcp_tool_call`) and B-G's `hook_degraded` has no §2.4 row by design; `Gated` true for B-A, B-B, B-E, B-F; B-C, B-D and **B-G** reported only — B-G structurally rather than softly, because no production evaluator can ever see one of its samples (a sample exists only when the daemon is unreachable, and `CheckBudgets`' only production caller is the daemon's own registry; V2-report §16.6 item 24). Every `Limit` is a `func(config.Config)` reading `cfg.Runtime.*` rather than a stored duration, and B-A's follows `runtime.hotPath.budgetMs`. **The row read "six … B-D never gated" until this audit** — B-G was added by the post-V2 hardening round, after V2-VERIFY was written |
...
> **Copy-paste hazard, third confirmation of the §2.2a ④ class:** the multi-target `-run` patterns in this table carry markdown's `\|` escaping, which Go's RE2 reads as a literal pipe — pasted raw they select **zero tests** and print `ok`. De-escape to `|` before running, and verify membership against `go test -list` first (V2-VERIFY's second pass observed the vacuous form directly on rows 03–06).
...
| V2-SP06-05 | Token classification (G10.2) | `go test -run 'TestClassify_All\|TestClassify_Table' ./internal/tokens/` | seven classes correct; SP-01's 14-case table unchanged by the inserted image-magic rule |
...
| V2-SP06-09 | **Object layer: 2-level fanout, zstd, dedup** | `go test -run 'TestPutBytes_FanoutLayout\|TestPutBytes_CompressionNone\|TestPutBytes_GlobalDedup\|TestPutBytes_IdenticalRootIsFree' ./internal/store/` | `objects/<h[0:2]>/<h[2:4]>/<h>.zst`; four file versions ⇒ 4 roots; `novel+reused == len(chunks)` on every line; every later version's `Novel` below v1's and ≤ the chunks spanning that version's diff (span derived from the fixtures, not the chunker); v2+v3 together add fewer objects than v1 alone wrote (`total − afterV1 < afterV1`) — restated for the real chunker at this checkpoint, see §2.6a ④; identical payload ⇒ `Novel==0`, no new `roots.jsonl` line |
...
| V2-SP06-21 | Flush + session index + append-only guard | `go test -run 'TestFlush\|TestFilesJSON_MaterializedByFlush\|TestAppendOnlyGuard_StoreFiles' ./internal/store/` (verify membership with `go test -list` — the original `TestFlush_` matched nothing, and the last-wins observable had no test behind it) | `sessions.jsonl` last-wins per session; second flush appends nothing; every `*.jsonl` this package writes refuses truncation |
| V2-SP06-22 | Store property suite | `go test -run Prop ./internal/store/` | `PropPutGetRoundtrip`, `PropOpenSpanMatchesSlice`, `PropDedupMonotone`, `PropChangedSinceIsExactlyHashInequality`, `PropMarkEncodedNeverDowngrades` all green (five, not six — `PropRedactIdempotent` never lived in `internal/store`; redact's own `TestRedact_Idempotent` and `FuzzRedactIdempotent` carry that property) |
...
| V2-SP06-26 | Conformance suites, zero skips | `go test -run 'Suite' ./internal/store/... ./internal/redact/... ./internal/tokens/...` ; `go test -run TestConformance_BehaviourBlocksActuallyRun ./internal/store/` (the mechanized W-1 blocker — the `Suite` pattern does not match it) ; `grep -rn "t.Skip" internal/store/storetest internal/redact/redacttest internal/tokens/tokenstest` | `RunStoreSuite` and `RunSegmentLogSuite` green; grep returns nothing |
...
> **All three ACTIONs below were discharged by the checkpoint** — dispositions in `plans/V2-report.md` §2.7a (ACTION 1 closed by `14a9668`, ACTION 2 by V2-MERGE-14, ACTION 3 superseded once the remote existed; see §16). The text is kept as the inbound record; do not re-execute it.
...
| V2-SP07-07 | `NodesAfter` total order | `go test -run 'TestNodesAfterOrdering\|TestNodesAfterReturnsFreshSlice\|PropNodesAfterMatchesFilter' ./internal/dag/` | live-only, freshly allocated, ordered by `(Pos, Turn, ID)` |
...
| V2-SP07-15 | **No selection authority** (closing note 3, mechanized) | `go test -run 'TestNoBooleanKeepAPI\|TestNoSelectionAuthorityNoteSurvives' ./internal/dag/ -v` (the second test carries the row's `doc.go` observable) | no exported function returns `map[NodeID]bool` / `[]bool`; no exported identifier matches `keepset\|dropset\|^keep\|^drop\|evict`; `doc.go` still contains the literal `NO SELECTION AUTHORITY` |
...
| V2-SP07-18 | **Slice latency** (§6.4 "sub-millisecond") | `go test -bench 'Slice' ./internal/dag/` and `go test -run TestSliceLatencyBudget ./internal/dag/` | `BenchmarkBackwardSlice5000` and `BenchmarkForwardSlice5000` **< 1 ms/op** on a 5 000-node / ~15 000-edge graph, graded by `TestSliceLatencyBudget` as the **fastest of 20** timed runs, not the median (`internal/dag/bench_test.go`: it sorts 20 durations and asserts `ds[0] < ceiling`). Read the gate that way — the least-contended sample is the deliberate choice, so a co-loaded host cannot fail a correct product, and it is why V2-report §16.4's wall-clock inventory classes this site as only *partly* co-load-safe; `BuildToolUse` < 3 µs **per AddNode+AddEdge pair** — `BenchmarkAddToolUse` measures the full §8.1 item-4 five-pair build, so the whole-op reading is ~5× the source budget and fails against the committed baseline itself (5.8–10 µs/op); SP-07's DoD (`plans/V2-SP-07-dependence-dag-and-slicing.md` line 923) budgets the pair, which the baseline meets at 1.2–2.0 µs (found at V2-VERIFY; same genus as §2.7a A) |
...
1. `go test -run TestBudgets_ ./internal/obs/` — B-A..B-G exist with config-driven limits (V2-SP01-13). The pattern deliberately covers both tests: `TestBudgets_AllSixPresentAndConfigDriven`, whose name predates B-G and which now asserts seven, and `TestBudgets_BGCoversTheDegradedSpoolAppend`.
...
go run ./test/replay --corpus testdata/sessions/synthetic --baseline testdata/baseline/phase0.json --phase 0 --ci --out v2-replay.json   # the flag is --out; --json does not exist
...
go run ./test/replay --corpus testdata/sessions/synthetic --write-baseline --baseline "$SCRATCH/a.json"
go run ./test/replay --corpus testdata/sessions/synthetic --write-baseline --baseline "$SCRATCH/b.json"
# there is no --to flag, and bare filenames would land in the repo root
diff a.json b.json      # Expect: identical, and both equal to testdata/baseline/phase0.json

# c) the ceiling and floor are real
go test -run 'TestSynthesize_StockBeatsNullOnEverySession|TestSynthesize_EveryCompactionHasDemands' ./internal/eval/
...
go test -run 'TestEveryCanonicalizer_Idempotent|TestEveryCanonicalizer_NeverGrows|TestRestore_RoundTrip|TestCorpus_StructuralProperties' ./internal/canon/
```
...
go test -v -run 'TestFreshBuildReportsModeFull|TestDeclaredProducerSetMatchesArchitecture' ./internal/contract/ ./internal/daemon/
...
go test -v -run 'TestNoBooleanKeepAPI|TestNoSelectionAuthorityNoteSurvives' ./internal/dag/
...
| `store.PutBytes` 100 KB cold / warm | ≤ 3 ms / ≤ 400 µs **(warm-with-redact is structurally over — see §2.6a ②'s V2 measurement; cold is syscall-dominated on Windows)** | `go test -bench 'PutBytes_100KB' -benchmem ./internal/store/` |
...
| `redact.Redact` 100 KB | ≤ 2 ms/op **(no-secret shape only; keyword/secret shapes are documented over-budget — §2.6a ②)** | `go test -bench . ./internal/redact/` |
...
| `dag.BuildToolUse` | < 3 µs per AddNode+AddEdge pair (the bench is the five-pair §8.1 item-4 build, so < 15 µs/op; the committed baseline is 5.8–10 µs/op — the old whole-op reading of this row failed against it) | `go test -bench BenchmarkAddToolUse ./internal/dag/` |
...
go run ./tools/devtool replay \
  ...
  --phase 0 --ci --out v2-replay.json   # the flag is --out; --json does not exist
...
`feat/sp08-observer-l0` and `feat/sp09-negative-knowledge` do not exist until every row of §2, every criterion of §3, every test of §4, every budget of §5 and every item of §6 passes on `verify/v2` **and** CI is green on the branch across all ten jobs — `verify`, `lint-windows`, `test` (×3 OS), `cover`, `crossbuild`, `bench-gate` (×3 OS), `replay-gate`, `plugin-validate`, `security`, `docs`.
...
- `TestGuard_Phase0BeforeStore` green at **every** wave-1 merge commit, not only the last: <n>/<n> first-parent commits PASS — all of them, whatever the count has grown to (21/21 at the cut)  [ ]
...
| V2-MERGE-21 | `plans/README.md` and this file agree on the wave-1 merge order, `develop` matches it, and the per-commit walk is green at **every** first-parent commit — not only the tip | | first-parent order observed: <list>; incoming-branch resolution commits present: <n>/6; walk result: <n>/<n> PASS (all, not a fixed count); counterfactual re-run and confirmed red: [ ] |
...
| V2-SP01-13 | obs histograms + B-A..B-G | | |
...
| V2-ALL-04 | CI green on verify/v2 (10 jobs, incl. lint-windows) | | bench-gate & replay-gate required: |
...
- [ ] CI green on verify/v2, all ten jobs: verify, lint-windows, test ×3, cover, crossbuild, bench-gate ×3, replay-gate, plugin-validate, security, docs — **or V2-ALL-04 recorded as environment-blocked with the reason** (no git remote). `lint-windows` is the tenth and is not optional: `verify` is ubuntu-only and the lint is partly blind there (V2-report §16.6 item 21)
