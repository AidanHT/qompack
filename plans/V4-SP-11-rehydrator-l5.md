...
// NewRehydrateService returns an observer.SessionStartRehydrator. It is nil-tolerant at every
// call site.
...
func BindRehydrate(o *daemon.Options, svc observer.SessionStartRehydrator)
```
...
§8.6 says: *"Every nested `CLAUDE.md` in a directory containing a pointer-set file."* The **shipped** contract is broader, and it is the one that binds: `internal/rules/rules.go:32-33` documents `NestedClaudeMD` as returning "every CLAUDE.md under root that lives in a directory containing (**or ancestor to**) a path in pointers", and the inherited Rule W-1 case `nested_claude_md_discovery` puts the file at `src/pkg/CLAUDE.md` with the pointer at `src/pkg/deep/thing.go` — an ancestor directory — and asserts `require.Len(t, matched, 1)` (`internal/rules/rulestest/suite.go:66-80`, fixture at :102-119). **Walk ancestors.** A containing-directory-only implementation returns an empty slice for that fixture and fails the suite the moment its stub probe stops skipping. Claude Code's own behaviour agrees: a nested `CLAUDE.md` governs its whole subtree, so a rule two directories above a pointer is exactly as lost after compaction as one beside it.
...
- In `internal/daemon/rehydrate_service.go` (this branch's own file), `BindRehydrate` attaches the service through the `Bind` seam, which is exactly what §5.4 provides for a wave-3 subplan:
  ```go
  func BindRehydrate(o *Options, svc observer.SessionStartRehydrator) {
      o.Bind(func(s *Services) { s.Rehydrate = svc.Compact })
  }
  ```
...
- In the daemon's startup sequence, after `store`, `negknow`, `dag` and `checkpoint.Reader` are constructed, build the service and call `BindRehydrate(o, svc)`.
...
4. `Tokens = Result.Tokens` **exactly** — the same number `runBudgetCase`, the hard-cap property test and `TestE2E_SessionStartCompactUnderBudget` check, so A1 and the unit tests cannot disagree about what "rehydration budget" means.
...
|---|---|---|
| **A1 — smaller budget** | `c.Driver.Policies["qompack-rehydrate"]["rehydration_tokens"]` vs `c.Driver.Policies["stock"]["rehydration_tokens"]`, over the 24-session synthetic corpus | `qompack < stock`, **and** `"qompack-rehydrate"` does not appear in `c.Driver.BudgetViolations` — the driver already records a violation for any policy whose keep-set overran the replay budget, so the per-session cap needs no second implementation here |
| **A2 — first-divergence improves** | `c.Driver.Policies[…]["first_divergence_turn"]` for `qompack-rehydrate` vs `stock` — the registered baseline policy, scored in the same run over the same ≥ 20 sessions carrying a `CompactionAt` point | `qompack > stock`; the error message prints both numbers so the replay-gate artifact is self-explaining |
...
- [ ] Merge `arch/rehydrate-item-kinds` into `develop`, then cut the feature branch from the merged tip.
...
- [ ] **Do not add a member to `daemon.Services`.** In the daemon startup sequence, build the service and call `BindRehydrate(o, svc)` so the already-shipped `Services.Rehydrate` seam is set — that is what makes `DeclareProducers` declare `contract.CAdditionalContext` (`internal/daemon/options.go:150-152`). Then pass the same `svc` into the observer construction site for the `source` switch. **That observer field assignment is the only line this branch changes in an SP-05/SP-08-owned file.**
...
Files: `internal/daemon/rehydrate_service.go`, `internal/daemon/rehydrate_service_test.go`, the daemon startup sequence and the observer construction site, `test/e2e/sessionstart_compact_test.go`.
