...
**§13 invariant 7.** "No network. No telemetry. No writes outside `.qompack/`" (plus `~/.qompack/` for the global layer). CI asserts the import graph and a runtime test asserts the write set. **Read the amended form, not this one:** the two-prefix shorthand predates §2.4's POSIX socket, which cannot live under either root, and invariant 7 is amended this round to "No writes outside the **product write set**", naming all five locations — the two `.qompack` roots plus the three socket locations `internal/ipc`'s `resolveFor` chooses between (§9's `writeset_test.go` below). `docs/security.md` states the amended set; enumerating two is not a stronger promise, it is a false one.
...
| 9 | `bloom` | `sketch.LoadWithLog` on `sketches/tried.bloom` succeeds (CRC + version); report `FillRatio`, `EstimatedFPRate`, `ResizeTarget` | on load failure **or** `needed == true`: `negknow.Ledger.RebuildBloom` (active records only, §8.3) |
| 10 | `sketches` | `touch.cms`, `explore.hll` load through `sketch.LoadWithLog` | corrupt → move to `<quarantine>/sketches/`, recreate empty from config (`NewCMS(epsilon, delta)`, `NewHLL(registers)`); they are caches, so this is lossless in kind |
...
| 13 | `retention` | `store.Stats` + a dry-run `store.GC(GCPolicy{RetainDays: d, RetainSessions: s, DryRun: true, Deadline: remaining})`, where `d`/`s` are `cfg.Store.Retention.Days`/`.Sessions` normally and **`-1`/`-1` under `--gc-all`**; report collectable bytes for the policy that ran; if the returned `GCReport.Truncated` is set, the finding says so and the figure is reported as a **lower bound**, never as the store's total — a pass whose deadline expires in the mark phase collects nothing and reports zero, which is byte-identical to a clean store unless the flag is carried. Note that this `Truncated` is the GC report's, not the fsck loop's | run the same GC with `DryRun: false` |
...
**Checks 9 and 10 read sketches through `sketch.LoadWithLog` and never through `sketch.Load`.** `test/guards/sketchload_test.go`'s `TestGuard_NoSilentSketchLoadOutsideItsPackage` parses every non-test `.go` file under `internal/`, `cmd/` and `test/` — skipping `testdata`, dot-directories and `internal/sketch` itself — and fails the build on any call expression spelled `sketch.Load`, because `Load` is `LoadWithLog` on a `logging.Nop()` (`internal/sketch/io.go`) and therefore writes no durable line anywhere. `fsck` hands it a **real** logger, the same construction `internal/cli/daemon.go` uses — `logging.New(paths.Of(root).Logs, logging.Info)` — and not `logging.Nop()`: the guard is syntactic, so a `Nop` passed to `LoadWithLog` would satisfy it while being exactly as silent as the form it forbids.

Classify the returned error by the sentinel it wraps, never by `core.ErrNotFound` — `LoadWithLog` returns that for absent, oversize, unreadable and corrupt files alike (§13 invariant 3: a sketch is a cache, so unreadable and missing are the same event upstream), so branching on it files bit rot under "expected":

- `errors.Is(err, fs.ErrNotExist)` — the file is **absent**: the ordinary cold start of a project's first session. `LoadWithLog` deliberately writes no `Loud` line for it, and neither does `fsck`; check 9 rebuilds from `records/eliminations.jsonl` and check 10 recreates empty, both as `FsckInfo`.
- `errors.Is(err, sketch.ErrCorrupt)` — a **CRC32C mismatch and nothing else** (`internal/sketch/errors.go`): the frame is structurally intact and its bytes are wrong. `LoadWithLog` has already written the single `Loud` line (`sketch corrupt — rebuilding from records`), so `fsck` records the finding and does not repeat it.
- `errors.Is(err, sketch.ErrUnsupportedVersion)` — `Ver == 0` or `Ver > FormatVersion`: a header/version failure, which is **neither** of the two above. The file is not absent and its bytes are not rotten; it was written by a different format version. Report it as its own class and quarantine it rather than folding it into the corrupt branch, whose remedy assumes the bytes are unrecoverable.
- `errors.Is(err, sketch.ErrTooLarge)` — refused on the `os.Stat` size before any read, so nothing was decoded and the frame's integrity is unknown.
...
tmpdir/qp-<hash8>.sock         (POSIX last resort, when neither of the two above fits sun_path)
```

**Five, not four.** `internal/ipc`'s `resolveFor` picks the socket path from three candidates in order — `$XDG_RUNTIME_DIR/qompack/<hash12>.sock`, `<os.TempDir()>/qompack-<uid>/<hash12>.sock`, and, when both exceed §2.4's 100-byte `sun_path` budget, the flat `<os.TempDir()>/qp-<hash8>.sock`. A project root deep enough to blow the budget writes that last one, which is an entry in `tmpdir/` *outside* the socket dir; a test listing only four prefixes therefore fails on exactly the deep-path machines `platform-matrix` exists to cover. Anything else — a stray file in `project/`, a dotfile in `home/`, or any other entry in `tmpdir/` — fails with the offending path printed. Windows adds no filesystem prefix at all (the named pipe is not a filesystem object under these roots), so the same five are the whole product commitment.
...
| `Confined write set` | The **five** prefixes of §9's `writeset_test.go`, verbatim — the two `.qompack` roots plus the three POSIX socket locations `internal/ipc`'s `resolveFor` chooses between — and what a violation would look like. The two user-global files are `config.json` and `calibration.json`; there is no `daemons.json` and the doc must not promise one |
...
**Two sibling assertions replace the `test/docs/security_test.go` SP-18 deleted when this file became SP-17's.** Owning the file means owning its coverage, and the headings alone do not carry it:

- `TestSecurityDocMatchesCIAssertions` — the `No network` section names `net/http`, `net/url`, `crypto/tls`, `internal/ipc`, `unix`, `tcp`, `os/exec` and `internal/daemon`, so the doc and the `security` job's allowlist cannot drift apart silently; the `Redaction` section names all **ten** rule descriptors `internal/redact/rules.go` actually ships (§5.22a enumerates nine because it writes `sk-`/`sk-ant-` as one family, and the tree splits them into `generic_sk_key` and `anthropic_key`).
- `TestSecurityDocStatesTelemetryOff` — the doc contains `runtime.telemetry.enabled` and states it is `false`.

SP-18's third deleted test, `TestSecurityDocStatesConfinedWriteSet`, is deliberately **not** restored as written: it required the doc to name `daemons.json`, which `grep -rn 'daemons.json' internal/ cmd/` shows exists nowhere. Its replacement asserts the five `writeset_test.go` prefixes and the two user-global files that do exist.
...
- **The nightly job stays deterministic.** `.github/workflows/nightly.yml`'s `replay-recorded` job (named `replay-live` until the post-audit fix round; the name described a mode no workflow has ever run) runs the ordinary driver over the recorded corpus (§6.3 tier 2) and is not given `--live`; 00-ARCHITECTURE is explicit that live mode is never run by any workflow. SP-17 changes no workflow here.
...
- [ ] Write the recovery half of `docs/security.md` — the last four `##` sections of the table in **Implementation spec §12**: `Degradation and recovery` (one paragraph per Qompack.md §12.3 row), `What this plugin cannot do` (the Qompack.md §12 list verbatim), `What to do if a secret reached the store`, `Reporting a vulnerability`. Add `TestSecurityDocSections` in `test/install/` asserting all eleven headings in order, plus `TestSecurityDocMatchesCIAssertions` and `TestSecurityDocStatesTelemetryOff` beside it — the coverage SP-18's deleted `test/docs/security_test.go` used to carry
...
16. Exactly one stamped version source: `grep -R -- '-X ' .goreleaser.yaml tools/devtool/` names `internal/core.Version` and no other version variable, and `grep -rn 'DefaultVersion' --include='*.go' .` returns nothing — `pluginmanifest.DefaultVersion` and `internal/pluginmanifest/version.go` exist nowhere in the tree. **This one is a merge blocker rather than a style note**: if either appears, the branch does not merge however green everything else is, because a second source ships a tagged binary whose `qompack version`, daemon lock, daemon status payload and MCP handshake still report the old number while every version assertion passes. A checkpoint or runbook that tells an operator to create one is to be corrected against this item, not obeyed.
...
- [ ] Commit count verified at **8** (within the 5–8 range): `git rev-list --count develop..feat/sp17-packaging-hardening-and-release` equals 8.
