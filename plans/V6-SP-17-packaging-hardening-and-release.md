...
| `internal/pluginmanifest`'s typed manifest source and `devtool plugin-validate`'s diff behaviour (SP-17 adds only `DefaultVersion` and the four assertions enumerated in Implementation spec §5a) | SP-01 |
...
// Files returns bundle-relative path → generated bytes for
// ".claude-plugin/plugin.json", "hooks/hooks.json", ".mcp.json", "commands/<name>.md".
func Files() map[string][]byte
```

> **Adaptation note, resolved:** if SP-01 named the accessor something other than `Files()`, use the existing accessor verbatim and do **not** rename it (Rule W-3). The single new exported symbol SP-17 introduces in that package is `DefaultVersion`.

### Produces (what later readers and CI rely on)

```go
// ── internal/pluginmanifest (one new symbol) ──────────────────────────────
// DefaultVersion is the single source of truth for the plugin version. It is stamped at
// release time via -ldflags -X and is otherwise the value committed in version.go.
var DefaultVersion = "0.1.0"

// ── internal/cli (new files; cli is a composition root per §3.2 and may import anything) ──
// Build identity, stamped by goreleaser via -ldflags -X.
var (
...
type BuildInfo struct {
    ...
    Manifest  string `json:"plugin_manifest_version"`
}
...
Replaces SP-01's skeleton. Pin the action to `version: '~> v2.6'` in the workflow (not a bare `~> v2`): `archives.formats` — the plural list form — is the non-deprecated spelling from v2.6 onward, and `TestGoreleaserConfigValid` asserts `goreleaser check` reports **zero deprecation notices**, which the singular `format` would trip.
...
before:
  hooks:
    - go mod download
    - go run ./tools/devtool verify-version --tag {{ .Version }}
...
builds:
  - id: qompack
    ...
    ldflags:
      - -s -w
      - -X github.com/qompack/qompack/internal/cli.buildVersion={{ .Version }}
      ...
      - -X github.com/qompack/qompack/internal/pluginmanifest.DefaultVersion={{ .Version }}
...
1. Resolve version: `--version` if given, else `pluginmanifest.DefaultVersion`. Reject anything not matching `^\d+\.\d+\.\d+(-[0-9A-Za-z.-]+)?$`.
2. Build (unless `--skip-build`): for each of the six targets, `go build -trimpath -ldflags "<same four -X plus -s -w>" -o dist/bin/qompack-<os>-<arch>[.exe] ./cmd/qompack` with `CGO_ENABLED=0`, `GOOS`, `GOARCH` set.
3. Generate manifest files by calling `pluginmanifest.Files()` — **never** by copying `plugin/**` from the worktree; the committed files are the diff target for `plugin-validate`, the generator is the source of truth. Assert the generated `.claude-plugin/plugin.json` has `"version": "<resolved version>"`.
...
|---|---|---|
| PV-1 | `plugin.json`'s `version` equals `pluginmanifest.DefaultVersion` | a `set-version` run regenerated only one of the two |
...
- `verify-version --tag <v1.2.3|1.2.3>`: strips a leading `v`, compares against `pluginmanifest.DefaultVersion`, against the first `## [x.y.z]` heading in `CHANGELOG.md`, and against `plugin/.claude-plugin/plugin.json`'s `version`. Any mismatch → exit 1 with all four values printed. This is the goreleaser `before` hook and the release workflow's `guard` job.
- `set-version <x.y.z>`: rewrites `internal/pluginmanifest/version.go` (`DefaultVersion = "x.y.z"`), regenerates `plugin/**` from the generator, and inserts a dated `## [x.y.z] - YYYY-MM-DD` heading into `CHANGELOG.md` above the previous top heading, moving everything currently under `## [Unreleased]` into it and leaving an empty `## [Unreleased]`. Run by a human in the release-prep commit; never by CI.
...
func Version() BuildInfo {
    return BuildInfo{
        ...
        Manifest: pluginmanifest.DefaultVersion,
    }
}
```

`qompack version` prints `qompack <version> (<commit12>, <date>) <go>/<os>/<arch>`; `--json` prints `BuildInfo` with `json.MarshalIndent(v, "", "  ")` plus a trailing newline. Always exits 0.

### 7. `internal/cli/fsck.go` — `qompack fsck`

Flags: `--repair`, `--deadline <dur>` (default `60s`), `--json`, `--strict`, `--quarantine-dir <path>` (default `<root>/.qompack/tmp/quarantine`).
...
| 2 | `objects` | Walk `objects/??/??/*.zst`. Assert: basename is 64 lowercase hex + `.zst`; the two fanout dirs equal hex[0:2] and hex[2:4]; zstd decodes with a 64 MiB decoder-memory cap; `sha256(plaintext)` equals the basename | move the object to `<quarantine>/objects/<name>.zst`, `Loud`, record |
| 3 | `roots` | For each line of `index/roots.jsonl`, every `ChunkRef.Hash` satisfies `store.Has` | none (report only — deleting a root would destroy the index; the remedy is re-observation) |
| 4 | `jsonl-tails` | For each append-only `*.jsonl` (`index/tool_use.jsonl`, `index/roots.jsonl`, `index/segments.jsonl`, `dag/deps.jsonl`, `records/eliminations.jsonl`, `checkpoints/MANIFEST.jsonl`, `pins/invariants.jsonl`): every line parses as JSON and the file ends with `\n` | copy the trailing partial bytes to `<quarantine>/tails/<file>.<unixms>.partial`, then truncate the file to the offset just after the last valid `\n` |
...
| 13 | `retention` | `store.Stats` + a dry-run `store.GC(GCPolicy{RetainDays: cfg.Store.Retention.Days, RetainSessions: cfg.Store.Retention.Sessions, DryRun: true, Deadline: remaining})`; report collectable bytes | run the same GC with `DryRun: false` |
...
|---|---|---|---|
| `binary` | `Version()`; `buildVersion == pluginmanifest.DefaultVersion` | mismatch → WARN | "development build — reinstall a released bundle" |
...
**`redaction_test.go`.** For each of the ten built-in rule families of §5.22a (private-key PEM, `AKIA`/`ASIA`, `ghp_`/`gho_`/`github_pat_`, `sk-`/`sk-ant-`, bearer tokens, `password=`/`secret=`/`token=`, `.env` value lines, JWT, credentialed connection strings), a fixture in `testdata/corpora/secrets/<rule>.txt` containing a syntactically valid fake secret with a unique 32-char marker embedded in it:
1. `store.PutBytes(fixture)` → `store.Open(root)` → read every chunk back → assert the marker is absent and `«redacted:` present.
...
**`.github/workflows/release.yml`** (replaces SP-01's placeholder):
...
jobs:
  ...
  attest:           # needs: install-gate — actions/attest-build-provenance@v2 run twice:
...
**`docs/security.md`** carries: the threat model (reads your code, writes to two gitignored directories, talks to nobody); the three network proofs and their platform coverage; the write-set prefix list; the redaction rule inventory and its choke point; the dependency allowlist including the `golang.org/x/sys` transitive allowance with its justification; the gosec/govulncheck posture and every suppression's reason; the §12.3 degradation table with a **recovery paragraph per row**; and the §12 "What this plugin cannot do" list reproduced verbatim.
...
All five are required checks on `develop` and `main` from this branch's merge onward.
...
| `TestVersionStamping` | build with the four `-X` flags set to known values | `qompack version --json` | all six fields exact; `plugin_manifest_version` equals the stamped version |
| `TestVerifyVersionRejectsMismatch` | CHANGELOG at 0.2.0, DefaultVersion 0.1.0 | `devtool verify-version --tag v0.2.0` | exit 1, all four values printed |
| `TestSetVersionRoundTrip` | clean tree | `devtool set-version 0.9.0` | version.go, plugin.json and CHANGELOG all updated; `verify-version --tag v0.9.0` exits 0; `plugin-validate` clean |
...
Every test named in §9 of the Implementation spec. Concretely enumerated inputs for `redaction_test.go`:

| Rule | Fixture content (marker in bold) | Expected |
|---|---|---|
| pem | `@@SEC_PEM_RSA_BEGIN@@\nMII**MARKER0001**…\n@@SEC_PEM_RSA_END@@` | `«redacted:pem»`, marker absent everywhere |
| aws | `AKIA**MARKER0002**XYZ` | `«redacted:aws»` |
| github | `ghp_**MARKER0003**` | `«redacted:github»` |
| anthropic | `sk-ant-**MARKER0004**` | `«redacted:anthropic»` |
| bearer | `Authorization: Bearer **MARKER0005**` | `«redacted:bearer»` |
| assignment | `password=**MARKER0006**` | `«redacted:assignment»` |
| dotenv | `API_KEY=**MARKER0007**` in a `.env` read | `«redacted:dotenv»` |
| jwt | `eyJhbGciOi….**MARKER0008**.sig` | `«redacted:jwt»` |
| connstring | `postgres://u:**MARKER0009**@h/db` | `«redacted:connstring»` |
| aws-session | `ASIA**MARKER0010**XYZ` | `«redacted:aws»` |
...
| `TestGuardRejectsExistingRelease` | fixture repo, tag on `main`, stub `gh release view` exiting 0 | `devtool release-guard` exits 1 naming the already-published release |
...
- [ ] Write failing tests first: `test/install/bundle_test.go` (`TestBundleLayoutPerPlatform`, `TestBundleLayoutUniversal`, `TestBundleDeterministic`, `TestSHA256SUMSFormat`, `TestBinarySizeBudget`, `TestPluginValidateNewAssertions`), `test/install/launcher_test.go` (`TestLauncherResolutionTable`, `TestLauncherPropagatesExitCode`, `TestLauncherExitsZeroOnMissingBinary`, `TestLauncherCmdArchSelection`, `TestLauncherPs1ArchSelection`, `TestShimOverheadBudget`), `internal/cli/version_test.go` (`TestVersionStamping`), `tools/devtool/version_test.go` (`TestVerifyVersionRejectsMismatch`, `TestSetVersionRoundTrip`), `test/install/goreleaser_test.go` (`TestGoreleaserConfigValid`)
...
- [ ] Add `internal/pluginmanifest/version.go` (`DefaultVersion = "0.1.0"`) and wire the plugin.json generator to it
...
- [ ] Add `internal/cli/version.go` (`buildVersion/buildCommit/buildDate`, `BuildInfo`, `Version`, `RunVersion`) and dispatch `qompack version`
- [ ] Add `tools/devtool/package.go` and `tools/devtool/version.go`; register tasks `package`, `verify-version`, `set-version`
- [ ] Replace `.goreleaser.yaml` with the six-target configuration
- [ ] Run `go test ./test/install/... -count=1` on this machine and `devtool package --json` — all green
...
- [ ] Write `test/security/{importgraph_test.go,network_test.go,writeset_test.go,telemetry_test.go,redaction_test.go,panic_test.go,alloc_test.go,tooling_test.go}` and the ten `testdata/corpora/secrets/*.txt` fixtures
...
- [ ] Write the audit half of `docs/security.md` (threat model, three network proofs, write-set prefixes, dependency allowlist incl. the `golang.org/x/sys` justification, gosec posture)
...
- [ ] Add `internal/cli/fsck.go` (13 checks, `FsckOptions`/`Finding`/`FsckReport`, human + `--json` rendering)
- [ ] Add `internal/cli/doctor.go` (16 checks, `DoctorOptions`/`Check`/`DoctorReport`)
- [ ] Wire both into the `main()` dispatch switch; keep hot-path subcommands free of any new init
...
- [ ] Write the recovery half of `docs/security.md`: one paragraph per §12.3 row, plus the §12 "What this plugin cannot do" list verbatim
...
- [ ] Write `tools/devtool/changelog_test.go` (`TestChangelogGeneration`, `TestChangelogCheckDetectsDrift`, `TestChangelogOmitsChore`) with a fixture git repo, and `test/install/workflow_test.go` (`TestGuardRejectsTagOffMain`, `TestGuardRejectsExistingRelease`, `TestGuardAcceptsCleanTag`, `TestWorkflowsLint`, `TestReleaseArtifactManifest`)
- [ ] Run `go test ./tools/devtool/... ./test/install/... -count=1` — must fail with "unknown task changelog" and "unknown task release-guard"
- [ ] Add `tools/devtool/changelog.go` (task `changelog`, `--from/--to/--check`) and `tools/devtool/guard.go` (task `release-guard`)
- [ ] Regenerate the `## [0.1.0]` body of the `CHANGELOG.md` created in commit 1 from the repo's history to date; leave `## [Unreleased]` empty
- [ ] Write `.github/workflows/release.yml` with the seven jobs in the stated `needs:` order (`guard` checks out with `fetch-depth: 0`); add the `package-gate` job to `ci.yml`
- [ ] Write `docs/release.md` (pipeline, determinism, attestation + `gh attestation verify`, rollback, upgrade path, Deferred: signing and marketplaces)
...
**Subagent C — security audit (commit 3).** Files: `test/security/**`, `testdata/corpora/secrets/**`, `.gosec.json`, `tools/devtool/security.go`, and the *audit half* of `docs/security.md`. Returns: the file set, the `go list -deps` allowlist it derived per `GOOS`, the gosec/govulncheck output, and any defect list in the same shape as B.

**Subagent D — fault injection (commit 4) and degradation (commit 6 tests).** Files: `test/fault/**`, `tools/devtool/fault.go`, and the *recovery half* of `docs/security.md`. Returns: the 54-case matrix result table, the nine degradation results, and a defect list.
...
**Subagent F — release tooling (commit 7 non-workflow parts) and install e2e (commit 8).** Files: `tools/devtool/{changelog.go,guard.go,install.go}`, the regenerated body of `CHANGELOG.md` (subagent A owns its creation), `test/install/{install_test.go,upgrade_test.go,uninstall_test.go,workflow_test.go}`, `docs/release.md`, `docs/install.md`. Returns: the file set and the generated CHANGELOG for review.
...
11. `qompack fsck --strict` exits 0 on a clean store, exits 1 on each of the nine seeded corruptions, and repairs all nine with `--repair`; the full pass over a 200 000-object store completes in < 60 s and is resumable under a short deadline.
12. `qompack doctor` runs all 16 checks in < 3 s with `--skip-mcp` and < 8 s with the MCP probe, in both human and `--json` modes.
13. `devtool verify-version` and `devtool changelog --check` are clean; `actionlint` is clean; a dry-run `goreleaser release --snapshot --clean` succeeds.
14. Full CI green on the branch: `verify`, `test`, `cover` (floors held), `crossbuild`, `bench-gate`, `replay-gate`, `plugin-validate`, `security`, `docs`, `package-gate`, `platform-matrix`, `fault-gate`, `install-gate`.
...
- [ ] Wave-5 file disjointness verified against SP-18: SP-17 touched `CHANGELOG.md`, `docs/security.md`, `docs/release.md`, `docs/install.md` and none of SP-18's seven files.
- [ ] All four new CI jobs (`package-gate`, `platform-matrix`, `fault-gate`, `install-gate`) are configured as required checks on `develop` and `main`.
