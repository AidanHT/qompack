...
- `analyzer.NewSelector` is still a stub. The closing-note-3 guard **changes shape** at V4 because
  `scheduler.PSelectionAvailable()` can now return `true` — submodular selection must still be inert,
  but the operative reason is now "the analyzer does not exist". Record which reason the guard asserts.
...
2. **PowerShell caveats.** `wc -l` does not exist; use `git rev-list --count <range>`. Where a row
   shows `grep`, use `Select-String`. Where a row shows `$TMP`, use `$env:TEMP`.
...
| V4-SP01-04 | `cmd/qompack/main.go` still under 150 LOC and dispatch-only after SP-12's three-block addition | `go test -run TestMainIsDispatchOnly ./test/guards/` ; `git diff --stat $(git rev-list --max-parents=0 HEAD) HEAD -- cmd/qompack/main.go` | green; the file remains a `switch os.Args[1]` with no business logic |
...
| V4-SP01-06 | `obs` budget table with **B-F in force** | `go test -run 'TestBudgets_AllSixPresentAndConfigDriven\|TestBudgetsMatchArchitectureTable\|TestCheckBudgetsNeverGatesBD' ./internal/obs/` | green; B-F's limit reads `runtime.budgets.mcpToolCallMs` (default 250) and `Gated` is true |
...
| V4-SP02-02 | **The policy registry now has five members** | `go test -run 'TestPolicyNames\|TestRegisterPolicy' ./internal/eval/ ./test/replay/...` | `PolicyNames()` includes `null`, `oracle`, `stock` plus the wave-3 registrations `qompack-l3` (SP-12, `test/replay/l3policy`), `stock-restore` and `qompack-rehydrate` (SP-11, `test/replay/phase3_rehydrate_test.go`). `oracle == 1.0`, `null == 0.0`, every other policy strictly between on all 24 sessions |
...
| V4-SP02-05 | Phase-0 baseline still byte-reproducible | `go run ./test/replay --corpus testdata/sessions/synthetic --write-baseline --to $TMP/v4-a.json` then `--to $TMP/v4-b.json`; `diff` | byte-identical, and both equal the committed `testdata/baseline/phase0.json`. `git diff --exit-code -- testdata/baseline/` clean |
| V4-SP02-06 | **Replay gate at `--phase 4`** — five phase checks now run | `go run ./tools/devtool replay -- --corpus testdata/sessions/synthetic --baseline testdata/baseline/phase0.json --phase 4 --growth $TMP/v4-growth.json --sketch $TMP/v4-health.json --max-wall 3m --ci` | exit 0; `sessions == 24`; `corpusTier == "synthetic"`; phase checks 0, 1, 2, **3** and **4** all run and all pass; `"regressions": []` |
...
| V4-SP02-09 | `internal/eval` import purity is unchanged by wave 3 | `go test -run TestImports ./internal/eval/` | `internal/eval` imports no `internal/` package outside `{core, paths, config, logging, obs}`. The wave-3 policies live in `test/replay/**`, which is a composition root |
...
| V4-SP04-04 | Canon normative properties unchanged | `go test -run 'TestPropIdempotence_EveryCanonicalizer\|TestPropNonGrowing_EveryCanonicalizer\|TestPropRestoreIsExactInverse' ./internal/canon/` | `Run(Run(x)) == Run(x)`; `len(canonical) ≤ len(input)`; `Restore` is a byte-exact inverse |
...
| V4-SP05-03 | **All nine contract assertions have real producers** | `go test -run 'TestFreshBuildReportsModeFull\|TestDeclaredProducerSetMatchesArchitecture' ./internal/contract/ -v` ; `go test -run TestV4_EveryContractAssertionHasARealProducer ./test/e2e/` | `ModeFull`; **zero** assertions at `Observed:"not-yet-implemented"`; `DeclareProducers` on a `Services` with `Rehydrate`, `MCPInitialized` and `PreCompact` set registers **all nine** ids. Five-declared/four-gated was the V2/V3 answer and is now a **regression** |
| V4-SP05-04 | `hook.additional_context_delivered` is a live SevCritical check | `go test -run 'TestSentinelRoundTrip\|TestAdditionalContextGatedUntilRehydrator\|TestSentinelNotObservedGivesTwoChances' ./internal/contract/` | with the producer declared the real check runs at `SevCritical`; the sentinel SP-05 mints is the one `rehydrate.Sentinel(sess, seq)` emits and the one `state/rehydrate-<sess>.json` carries |
...
| V4-SP05-06 | `mcp.server_registered` observes a real `initialize` | `go test -run 'TestMCPInitializedSeamFlipsAfterInitialize\|TestInitializedWritesContractHistory' ./internal/daemon/ ./internal/mcp/` ; `go test -run TestE2ESelfTest ./test/e2e/` | `Services.MCPInitialized()` false before the first `initialize`, true after; `state/contract.json` carries `MCPInitialized: true`; `qompack self-test --json` exits 0 with `mode == "full"` |
...
| V4-SP05-12 | **B-A/B-B/B-D/B-E hot-path harness with the full wave-3 resident set** | `go run ./tools/devtool bench-hotpath --iterations 2000 --hook observe-tool --warm-daemon --json v4-hotpath.json` and again `--hook checkpoint --iterations 200` | `B-A.pass == true` (p99 < 15 ms), `B-B.pass == true`, **`B-E.pass == true` (p99 < 2 s, now a real `PreCompact`)**; `b_a_method` and `spawn_floor_ms` present; B-D reported, never gated. See §5 |
...
| V4-SP06-07 | Phase-1 dedup ratio unchanged by wave 3 | `go test -run 'TestPhase1ExitCriterion_ReadHeavy\|TestStats_DedupRatio' ./internal/store/ -v` ; `go test -run TestPhase1_DedupRatioReadHeavy ./test/e2e/ -v` | `DedupRatio ≥ 4.0` at both levels. Record both numbers and compare against the V3 figures |
...
| V4-SP09-02 | **`IngestMCP` is now driven by the real `record_eliminated` tool** | `go test -run 'TestIngest_MCP' ./internal/negknow/` ; `go test -run 'TestRecordEliminated' ./internal/mcp/` | evidence stored in the store and resolvable; `depends_on` resolved to the newest roots; unresolved deps reported not dropped; `Source == SourceMCP`; scope defaults from `eliminations.defaultScope`; session-scope without a session is an error |
...
| V4-SP10-20 | **Checkpoint benchmark budgets, including B-E** | `go test ./internal/checkpoint/ -bench . -benchmem -run '^$' -benchtime 2s` ; `go run ./tools/devtool bench-hotpath --hook checkpoint --iterations 200 --warm-daemon --json v4-be.json` | `BenchmarkFinalize` mean < 50 ms (40 segments, 64 decisions, 400 pointers); `BenchmarkAdvanceSegment` < 25 ms; `BenchmarkTruncate` < 5 ms; `BenchmarkExtractDecisions` < 20 ms; `BenchmarkStripInjections` < 2 ms; **B-E p99 < 2 s on linux, macos and windows** |
| V4-SP10-21 | **SP-10's own exit gate — frontier advancement shrinks the residual span** | `go run ./tools/devtool replay --corpus testdata/sessions/synthetic --filter multi-compaction --set checkpoint.frontier.advanceOnSegmentClose=false --json $TMP/v4-frontier-off.json` then the same with `=true --json $TMP/v4-frontier-on.json` | `on.Score.ResidualSpan.P50 ≤ 0.70 × off.Score.ResidualSpan.P50` (at least a 30 % reduction), and `Score.Divergence` does not regress by more than 2 % (§11.3). Record both P50s |
...
| V4-SP10-23 | Coverage floors | `go run ./tools/devtool cover` | `internal/checkpoint` ≥ **90 %**, `internal/pins` ≥ **90 %** |
...
| V4-SP12-05 | Ski-rental threshold computed, never written | `go test ./internal/scheduler/ -run TestSkiRentalShouldWrite -v` ; `grep -rn "12\.5" internal/ --include=*.go \| grep -v _test.go` | threshold `w/r == 12.5` at `r=0.1, w=1.25`; 12 ⇒ false, 12.5 ⇒ false, 12.6 ⇒ true; `r=0` or `w=0` ⇒ false; the grep returns nothing |
...
| V4-SP12-19 | **O3 idle-time background work, gated by decision and by degradation** (§8.4) | `go test ./internal/daemon/ -run 'TestIdleTask\|TestIdleActingTaskSkippedInDegradedPassive\|TestIdleWorkBindsDaemon\|TestIdleTasksRegistered' -v` | exactly six tasks in priority order `act.advance_frontier, precompute_slice, refresh_delta, rebuild_bloom, compact_dag, gc` at priorities 10…60; **`act.advance_frontier` is the only prefixed task and is suppressed in `ModeDegradedPassive`** while the other five keep running (§12.1 "no scheduler-initiated checkpoints"); every task inert until an `Evaluate` has placed it in `Decision.Background`; all six inert when `backgroundWork=false`; a nil ledger is safe; GC receives a positive deadline ≤ the run budget with retention from config; cancellation honoured within 5 ms |
...
go run ./tools/devtool replay -- --corpus testdata/sessions/synthetic \
  --baseline testdata/baseline/phase0.json --phase 3 --ci --json $TMP/v4-phase3.json
jq '.policies["stock-restore"].rehydration_tokens, .policies["qompack-rehydrate"].rehydration_tokens,
    .policies["qompack-rehydrate"].divergence.first_divergence_turn,
    .policies.stock.divergence.first_divergence_turn' $TMP/v4-phase3.json
```

| Assertion | Definition | Pass condition |
|---|---|---|
| **A1 — "at a *smaller* rehydration budget than stock"** | `median(qompack-rehydrate.Score.RehydrationTokens)` vs `median(stock-restore…)`. `stockRestorePolicy` reproduces §2.4 step 7 exactly: top-5 recently-read files at 5K each under a 50K cap plus invoked skills at 5K each under a 25K cap; `qompackRehydratePolicy` reports `rehydrate.Build`'s `Result.Tokens` and nothing else | `qompack < stock` **and** `qompack ≤ runtime.rehydrate.maxTokens` (12 000) on **every** session. Record both medians and the per-session max |
| **A2 — "first-divergence turn index improves against Phase 0 baseline"** | `Score.Divergence.FirstDivergenceTurn`, median over the ≥ 20 sessions carrying a `CompactionAt` point, against SP-02's committed `testdata/baseline/phase0.json` | `median(qompack) > median(baseline)`; the message prints both numbers so the gate artifact is self-explaining. Record both and the delta in turns |
| **A3 — "measured reduction in summarization-call input tokens"** | `residual = Σ tokens of turns after Ref.Frontier`; `stockSpan = Σ tokens of all turns up to the compaction point` | `residual ≤ 20 000` (`checkpoint.frontier.maxResidualTokens`) **and** `1 − residual/stockSpan ≥ 0.5` on the multi-compaction sessions. Record the ratio per session |
...
**What changes at V4.** SP-11 ran A1/A2/A3 with `Frontier` values read from checkpoint *goldens*,
because SP-10 was a same-wave sibling. Re-run them with `Ref.Frontier` produced by SP-10's real
...
| 4. Nothing scattered before `p` | V4-SP12-10 and `TestPhase4_PSelectionGateHonoured`; record which condition the guard actually asserts at V4 |
...
| 9. Every latency budget is measured, not assumed | §5 in full — B-A, B-B, B-C, B-D, B-E and, newly, **B-F** |
...
`## 6b. Skill index`, `## 7. No longer in context`, `## 8. Retrieval`, and ends inside the tags with
`rehydrate.Sentinel("v4-1", 1)`. **The budget:** its estimated token count is **≤ 12 000**
...
and the matching sentinel; no decompressed object contains a secret literal.
...
go run ./tools/devtool bench-hotpath --iterations 200  --hook checkpoint    --warm-daemon --json v4-be.json
```
...
`MCPInitialized` from `state/contract.json` after a real `initialize`. Removing any one producer from
...
| **B-E** | `checkpoint_finalize`: `PreCompact` entry → exit | **p99 < 2 s** | `go run ./tools/devtool bench-hotpath --iterations 200 --hook checkpoint --warm-daemon --json v4-be.json` ; plus `go test -bench BenchmarkFinalize -run '^$' ./internal/checkpoint/` | | | | **Hard fail. First real measurement** — `Services.PreCompact` is SP-10's `Compactor`, not nil |
...
| **Store dedup ratio, read-heavy** | **≥ 4:1** | `go test ./internal/store/ -run TestPhase1ExitCriterion_ReadHeavy -v` ; `go test ./test/e2e/ -run TestPhase1_DedupRatioReadHeavy -v` | In force |
...
go run ./test/replay --corpus testdata/sessions/synthetic \
  ...
  --growth $TMP/v4-growth.json --sketch $TMP/v4-health.json --signoff $TMP/v4-signoff.txt \
  --max-wall 3m --ci --json $TMP/v4-replay.json          # expect exit 0 and "regressions": []
...
- **A regression discovered at a verification checkpoint is fixed, not signed off.** The `sign-off:`
  ...
  has defeated its purpose. If a metric genuinely must move, the sign-off belongs on the wave-4 PR that
  trades it, naming that exact metric.
...
- `bloom_fp_rate` and `bloom_fill_ratio` are subject to the same 2 % rule **plus** the hard
  `EstFPRate > 0.10` ceiling. `benchstat` (§5.5) is the micro-benchmark analogue at 10 %/25 %.
...
Record the completion report in the merge commit body or as `docs/adr/0101-v4-verification.md` — the
numbers are the input to V5's regression comparison and must not live only in a terminal scrollback.
...
- [ ] `verify/v4` merged into `develop` with `--no-ff`; `develop` tagged `v0.3.0`; the completion
      report recorded in the merge body or `docs/adr/0101-v4-verification.md`.
