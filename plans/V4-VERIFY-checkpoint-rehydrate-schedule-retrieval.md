...
| V4-SP02-06 | **Replay gate at `--phase 4`** — five phase checks now run | `go run ./tools/devtool replay --corpus testdata/sessions/synthetic --baseline testdata/baseline/phase0.json --phase 4 --growth $TMP/v4-growth.json --sketch $TMP/v4-health.json --max-cpu 3m --ci --out $TMP/v4-phase4.json` | exit 0; `sessions == 24`; `corpusTier == "synthetic"`; phase checks 0, 1, 2, **3** and **4** all run and all pass; `"regressions": []`. **No bare `--` and no `--json`**: the separator makes Go's `flag` stop parsing (the driver now refuses the leftovers with exit 2, `TestReplayDriver_LeftoverArgumentsAreBadInput`), and the report flag is `--out`. `--max-cpu` is the cost bound — the one that used to be spelled `--max-wall 3m` — while `--max-wall` keeps its 15 m liveness default |
...
| V4-SP12-05 | Ski-rental threshold computed, never written | `go test ./internal/scheduler/ -run TestSkiRentalShouldWrite -v` ; `go run ./tools/lint/nomagic ./internal/scheduler` | threshold `w/r == 12.5` at `r=0.1, w=1.25`; 12 ⇒ false, 12.5 ⇒ false, 12.6 ⇒ true; `r=0` or `w=0` ⇒ false; `nomagic` exits 0, which is the gate that actually enforces §11.6 here. **Do not use a tree-wide `grep -rn "12\.5" internal/`**: it already returns `internal/sketch/doc.go`, a doc comment enumerating `nomagic`'s forbidden set, and `nomagic` inspects literals rather than comments — a verifier running the grep records a FAIL for a non-defect or "fixes" a correct comment. If a grep is wanted anyway, scope it to the owning package and drop comment lines: `grep -rn "12\.5" internal/scheduler --include=*.go \| grep -v _test.go \| grep -v '^\s*//'` |
