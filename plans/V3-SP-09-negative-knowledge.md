...
**What exists in the repo when you start.** `develop` carries all of wave 0 and wave 1. From SP-01: `internal/core` (`Hash`, `HashBytes`, `Dep`, `ChunkRef`, `Clock`, the sentinel errors), `internal/paths` (`Norm`, `Key`, `AppendOnly`, `CreateNew`, `WriteAtomic`), `internal/config` (the full Appendix C schema including `EliminationsCfg` and `SketchesCfg`), `internal/logging`, `internal/obs`, `internal/testutil` (`NewProject`, `FakeClock`), the compiling `ErrNotImplemented` stub of `negknow.Ledger`, the `negknowtest` conformance suite with its behaviour tests `t.Skip`ped (Rule W-1), and `testdata/golden/contracts/negknow/`. From SP-03: a real `internal/sketch` with `Bloom`, `NewBloom`, `RebuildBloom(capacity, fpRate, keys iter.Seq[[]byte])`, `FillRatio`, `EstimatedFPRate`, `ResizeTarget`, `Save`/`Load`. From SP-06: a real `internal/store` with `ChangedSince([]core.Dep)`, `FileHistory`, `PutBytes`, and the CRC-checked object store. From SP-07: a real `internal/dag` with `AddNode`, `AddEdge`, `Node`, `In`, `Out`, `NodesAfter`, `BackwardSlice`.
...
| Item | Owner |
|---|---|
| `sketch.Bloom` internals, the Appendix A sizing math, `FillRatio`, `EstimatedFPRate`, `ResizeTarget`, `sketch.RebuildBloom`, `Save`/`Load`, the versioned CRC header | **SP-03** |
...
func AppendOnly(p string) (*os.File, error)    // O_WRONLY|O_APPEND|O_CREATE; ErrAppendOnly on O_TRUNC
func CreateNew(p string) (*os.File, error)     // O_EXCL; used for the bloom staging file
...
**One documented assumption, and its fallback.** 00-ARCHITECTURE §5.2 names the `Counter` and `Gauge` *types* but not their methods. This subplan assumes SP-01 ships `Counter.Inc()` / `Counter.Add(int64)` and `Gauge.Set(float64)`, and every `count(...)` helper in this file expands to `l.m.Counter(name).Inc()`. If the shipped method names differ, adapt these call sites — never `internal/obs`, which Rule W-3 puts off limits. That is a mechanical rename confined to `internal/negknow`, not an amendment.
...
func (d Descriptor) Key() []byte      // 32 bytes; HashBytes("qompack.neg.v1", …) over all four fields
// Descriptor carries its own JSON codec so ReasonHash renders as "sha256:<hex>" whether the
// descriptor is marshalled standalone (SP-13) or nested inside a Record (SP-10).
...
func NodeIDFor(r Record) dag.NodeID             // "elimination:" + r.ID
func FileNodeID(pathKey string) dag.NodeID      // "file:" + paths.Key(path)
...
```go
const (
    domainDescriptor = "qompack.neg.v1"
...
func (d Descriptor) Key() []byte {
    ...
    b.WriteString(sanitizeField(d.ApproachClass));  b.WriteByte(fieldSep)
    b.WriteString(d.ReasonHash.String())
    h := core.HashBytes(domainDescriptor, b.Bytes())
...
**On-disk JSON.** `Record` carries explicit `MarshalJSON`/`UnmarshalJSON` so the wire form matches §8.5 — lowercase `path`/`hash` keys inside `depends_on`, `"sha256:…"` text for every hash, and a string for `source`. This also means SP-10's `checkpoint.Checkpoint.Eliminated []negknow.Record` serializes into `eliminated[]` in the shape §8.5 prints, with no work on SP-10's side.

To be precise about "matches §8.5": the wire form is a **superset**. All seven keys §8.5 shows — `target`, `approach`, `reason`, `evidence`, `depends_on`, `scope`, `status` — appear with exactly §8.5's names, positions relative to one another, and value shapes. `id`, `session`, `ts`, `descriptor` and `source` are additive fields §8.5's illustrative snippet elides; they are required for the log to be replayable and for `already_tried` to be answerable, and no consumer of the checkpoint may reject unknown keys (00-ARCHITECTURE §11.3 makes unknown-key tolerance the house rule). A golden test pins the full byte sequence; a second assertion extracts the seven §8.5 keys and compares them against a literal transcribed from §8.5, so drift against the design document is caught independently of drift against our own golden.
...
func recordID(sess core.SessionID, ts core.UnixMilli, d Descriptor) string {
    ...
    return "elm_" + hex.EncodeToString(h[:])[:12]
}
```
...
Two line kinds, discriminated by `op`:

```go
type logKind string
const (opAdd logKind = "add"; opStale logKind = "stale")
...
Byte-for-byte, an `add` line (one line, `\n`-terminated, no interior newlines, `json.Encoder` with `SetEscapeHTML(false)`):

```
{"op":"add","rec":{"id":"elm_9f4c1a2b7e03","session":"sess-01","ts":1754899200000,"target":"src/auth.ts:refreshToken","approach":"widen pool timeout","reason":"pgbouncer 1.18 ignores it in transaction mode","descriptor":{"normalized_path":"src/auth.ts","symbol":"refreshToken","approach_class":"widen-pool-timeout","reason_hash":"sha256:6b1f2c8d4a9e0357bd18f6c2a47e93015d8b7f30c1a692e4b05d3f8a7c216e94"},"evidence":"sha256:a3f2c07b415d6e89032f7a1c8b4d5e60927fa3b18c0d4e5f6172839a4b5c6d7e","depends_on":[{"path":"docker-compose.yml","hash":"sha256:11aa22bb33cc44dd55ee66ff7788990a0b1c2d3e4f50617283945a6b7c8d9e0f"},{"path":"package-lock.json","hash":"sha256:22bb33cc44dd55ee66ff7788990a0b1c2d3e4f5061728394aa6b7c8d9e0f11aa"}],"scope":"project","status":"active","source":"mcp"}}
```

Both dep hashes above are exactly 64 hex characters after the `sha256:` prefix. Key order in the emitted line is the `recordWire` struct field order, which `encoding/json` preserves; `testdata/golden/contracts/negknow/eliminations.golden.jsonl` is the byte-level authority.

A `stale` line:

```
{"op":"stale","id":"elm_9f4c1a2b7e03","ts":1754985600000,"because":["docker-compose.yml: dependency hash changed from sha256:11aa22bb33cc"]}
```
...
3. `op == "add"`: if `Rec.ID` is already present, the **first** occurrence wins and the duplicate is counted in `negknow.log.duplicate_add`; otherwise append to the slice and index by ID.
4. `op == "stale"`: if the ID is known, set `Status = StatusStale`, `StaleSince = TS`, `StaleBecause = Because` (replacing, not appending, so a re-flip is idempotent). If unknown, count `negknow.log.orphan_stale` and continue.
5. Any other `op` value: skipped, counted in `negknow.log.unknown_op`, logged at `Warn`. This is the forward-compatibility path for a newer plugin version.
6. Returns records in log order, the ID→index map, and the count of lines read.

Appends are made under the ledger's mutex through a single held `*os.File`; each append is one `Write` of a fully-built `[]byte` ending in `\n` (atomic for a single sub-`PIPE_BUF` write on POSIX and a single `WriteFile` on Windows). `Close()` closes the handle. There is no `fsync` per append: the WAL/durability boundary for the plugin is the daemon's spool (00-ARCHITECTURE §2.4), and a lost tail line costs at most one elimination, never a corrupt index.
...
type ledger struct {
    ...
    f        *os.File                   // append handle on records/eliminations.jsonl
    closed   bool                       // Close is idempotent
    pending  bool                       // a bloom rebuild is owed (rebuildOnStale == nextIdle)
    seq      int                        // rebuild generation, loaded from and persisted to
                                        // state/negknow.json so generations are monotonic
    deps     Deps
    log      logging.Logger
    m        obs.Registry
    clk      core.Clock
...
3. Read `<root>/.qompack/state/negknow.json` if it exists and initialize `l.seq` from its `rebuild_seq` field; a missing, unreadable or unparseable file means `l.seq = 0` and is logged at `Debug`, not `Warn` — a fresh project has no such file.
...
5. Bloom acquisition:
   ...
   - `b == nil` → `nb := sketch.NewBloom(cfg.Sketches.Bloom.Capacity, cfg.Sketches.Bloom.FpRate)` then `sketch.Load(filepath.Join(root, ".qompack", "sketches", "tried.bloom"), nb)`. `core.ErrNotFound` (missing or CRC-failed, per §5.7) is not an error here: it triggers step 6's immediate rebuild.
...
Helpers used above: `count(s)` is `l.m.Counter("negknow.query."+s).Inc()`; `hex(b)` is `hex.EncodeToString(b)`; `filter` is an inline loop over `idx` into `l.recs`. The whole body runs under `l.mu.RLock()`, and the histogram `negknow.query` is observed on every path via `defer`.
...
7. **Persist** (the §3.3 replacement procedure, the only legal way `tried.bloom` is replaced):
   - `l.seq++`; marshal with `nb.MarshalBinary()`. `l.seq` was initialized from `state/negknow.json` at `Open`, so generations stay monotonic across restarts.
   - Write to `<root>/.qompack/tmp/tried.bloom.<seq>` with `paths.CreateNew`, `Write`, `Sync`, `Close`. If that exact path already exists — the leftover of a rebuild that crashed between create and rename — `os.Remove` it first and retry `paths.CreateNew` once; a second failure aborts the persist block.
   - If `sketches/tried.bloom` exists: remove any existing `sketches/tried.bloom.*.bak`, then `os.Rename(tried.bloom → tried.bloom.<seq-1>.bak)`, where `<seq-1>` is the generation of the file being displaced — this is §3.3's "the previous file is kept as `tried.bloom.<seq>.bak` for one generation", with `<seq>` read as the displaced generation. Exactly one `.bak` exists at any time.
   - `os.Rename(tmp → sketches/tried.bloom)`; on POSIX, `fsync` the `sketches` directory.
   - Persist `{"rebuild_seq": <seq>, "records": <n>, "ts": <ms>}` to `<root>/.qompack/state/negknow.json` with `paths.WriteAtomic`.
   - Any failure in this block: log `Loud`, keep the **new in-memory** bloom (it is correct), leave the on-disk file untouched, return the error alongside the bloom and health. The caller is not obliged to fail.
...
```go
func NodeIDFor(r Record) dag.NodeID  { return dag.NodeID("elimination:" + r.ID) }
func FileNodeID(p string) dag.NodeID { return dag.NodeID("file:" + paths.Key(p)) }

func (l *ledger) emitDAG(r Record) // best-effort; every error logged at Debug and swallowed
```

Adds one node `{ID: NodeIDFor(r), Kind: dag.KindElimination, Turn: 0, TS: r.TS, Ref: r.ID, Root: r.Evidence}` and, when `r.Desc.NormalizedPath != ""`, one edge `{From: NodeIDFor(r), To: FileNodeID(r.Desc.NormalizedPath), Kind: dag.EdgeExplains, Weight: 1}`. This is what lets SP-11 rank eliminations by slice score (§8.6 item 3) and SP-15 count them as high-Δ content (G6.3). The two ID conventions are pinned in `testdata/golden/contracts/negknow/node-ids.json` so a divergence from SP-07/SP-08's naming is caught at the wave-2 verification checkpoint rather than at runtime.
...
| `sketch.Load` returns `core.ErrNotFound` (missing or bad CRC) | rebuild from active records immediately; if that fails, empty bloom + `pending` |
...
| Rename failure during bloom persistence | `Loud`, keep the new in-memory bloom, leave disk untouched, return the error |
...
| Fixture | Contents |
|---|---|
| `testdata/golden/contracts/negknow/eliminations.golden.jsonl` | 6 lines: 4 `add` (2 project-scope, 2 session-scope, one with no `depends_on`, one with 2 deps), 1 `stale`, 1 unknown-`op` forward-compat line |
| `testdata/golden/contracts/negknow/record.golden.json` | the §8.5 example record marshalled by `Record.MarshalJSON`, byte-compared; a second assertion projects out §8.5's seven keys and compares them against a literal transcribed from the design document |
| `testdata/golden/contracts/negknow/descriptors.golden.json` | 24 `(target, approach, reason)` → `(path, symbol, class, reason_hash_hex, key_hex, match_key_hex)` rows, including at least one symbol-less target, one `#`-separated target, one drive-letter target and one `unclassified` approach — pins hashing stability across platforms and Go versions |
...
| `testdata/golden/contracts/negknow/node-ids.json` | `{"elimination":"elimination:elm_9f4c1a2b7e03","file":"file:src/auth.ts"}` |
...
| `TestKey_SeparatorInjection` | target `"a\x1Fb"` (no `:` or `#`) vs the pair path `"a"` / symbol `"b"` | distinct keys: `sanitizeField` (below) replaces every `0x1F` in a descriptor field with `0x20` before hashing, so the first case hashes `"a b"` as the path with an empty symbol and cannot collide with the second |
...
| Test | Setup / input | Expected |
|---|---|---|
| `TestRecordJSON_Golden` | the §8.5 example record | `MarshalJSON` output equals `record.golden.json` byte-for-byte |
...
| `TestSourceKind_RoundTrip` | all four kinds | `ParseSourceKind(k.String()) == k`; `ParseSourceKind("nope")` errors |
...
| `TestRecordID_Stable` | fixed session/ts/descriptor | `"elm_"` + 12 lowercase hex, identical across runs and platforms |
...
| `TestReplayLog_DuplicateAdd` | two `add` lines with the same ID and different reasons | first wins; `negknow.log.duplicate_add == 1` |
...
| `TestRebuildBloom_Persistence` | pre-existing `tried.bloom` | after rebuild: `tried.bloom` is the new file; exactly one `tried.bloom.*.bak` exists; `.qompack/tmp` has no leftovers; `state/negknow.json` has `rebuild_seq == 1` |
| `TestRebuildBloom_OneBakGeneration` | three consecutive rebuilds | exactly one `.bak` file after each |
| `TestBloomLoadFailure_RebuildsFromRecords` | corrupt `tried.bloom` (flipped CRC byte) + a valid records file | `Open` succeeds; `Query` on a recorded elimination returns `AnswerActive`; `Loud` fired; `negknow.bloom.rebuilds == 1` |
...
- [ ] Add `internal/negknow/doc.go`, `classify.go`, `descriptor.go`.
- [ ] Add `testdata/golden/contracts/negknow/descriptors.golden.json` (24 rows), generated once by a `-update` flag on `TestDescriptorKey_Golden` and then committed.
...
records/eliminations.jsonl is the source of truth (§8.3 item 1). Status changes
are appended as op:"stale" lines rather than rewrites, so the §7.4 append-only
invariant holds for the one file the whole feature rests on. Custom JSON keeps
the on-disk shape byte-identical to the eliminated[] block of §8.5.

Refs: SP-09, G6.1, §7.4, §8.3, §8.5
```

- [ ] Write `record_test.go` and `log_test.go` first; run and watch them fail.
- [ ] Add `record.go`, `log.go`.
- [ ] Add `testdata/golden/contracts/negknow/record.golden.json`, `eliminations.golden.jsonl`, `corrupt.jsonl`.
...
site in internal/. Persistence follows the §3.3 replacement procedure with one
.bak generation, and a corrupt or undersized bloom rebuilds rather than lying.

Refs: SP-09, G6.2, §6.2, §8.3, §12 (bloom saturation), Appendix A
```

- [ ] Write `bloom_test.go` first (13 cases including the single-call-site grep test); run and watch them fail.
...
- [ ] Write `ingest_test.go` (13 cases) and `detector_test.go` (11 cases) first; run and watch them fail.
...
- [ ] Add `docs/adr/0009-negative-knowledge-bloom-as-cache.md` recording, one short section each: two bloom keys per record and why; `stale` as an append-op rather than a rewrite; coarse approach classes as a deliberate recall/precision trade; why `nextIdle` rebuild cannot cause a wrong answer; why the rebuild honours the configured capacity instead of over-allocating, so `tried.bloom` stays at Appendix A's ~12 KB; why identity dedup is keyed on `(session, scope, Desc.Key())` rather than on the record ID; and why `blind` mode never rebuilds.
...
| **S2 — persistence** | `log.go`, `log_test.go`, the `MarshalJSON`/`UnmarshalJSON`/`normalizeRecord`/`recordID` bodies in `record.go`, `record_test.go`, `record.golden.json`, `eliminations.golden.jsonl`, `corrupt.jsonl` | `ledger.go`, `descriptor.go` | Files plus the byte-exact golden fixtures |
...
- [ ] `security` job green: no `net/http`, `net/url`, `crypto/tls`, `os/exec` import from `negknow`; the write set is confined to `.qompack/records/`, `.qompack/sketches/`, `.qompack/state/` and `.qompack/tmp/`.
