...
3. **Merge in order.** When all sessions finish, merge the wave's branches into `develop` with `--no-ff`, in this order:
   ...
   These orders are not arbitrary and are not free to reorder. **Wave 1's order is the one currently enforced as a test**, and it is the only one: `test/guards/buildorder_test.go` holds exactly two build-order guards. `TestGuard_Phase0BeforeStore` fails while `internal/store` is real and `internal/eval` is still a stub, because closing note 1 of `Qompack.md` says a store that ships before its baseline can never be measured against one — that is the **SP-02 before SP-06** constraint, and merging wave 1 in the wrong order leaves `develop` failing at an intermediate commit even though every branch is individually green. Wave 1's other binding constraint, **SP-06 after SP-03 and SP-04**, comes from `V2-SP-06-content-addressed-store.md`'s exit criteria. Wave 1's row was corrected during that wave's merge, which is when the second constraint was discovered; see `V2-VERIFY-primitives-store-dag-and-baseline.md`'s `V2-MERGE-21`, which scopes the enforcement claim to wave 1 for the same reason.

   The second guard, `TestGuard_StoreAndNegknowBeforeCheckpoint`, is a **cross-wave** constraint, not an intra-wave one: `store` lands in wave 1 and `negknow` in wave 2, so every permutation of wave 3 satisfies it and it discriminates none of them. **The wave 2–5 orders above are stated conventions, not enforced ones.** They follow from the consuming relationships each subplan's own prerequisites section states — SP-09's elimination ledger lands before SP-08's observer, which imports it (§3.2); SP-10's checkpoint schema lands before SP-11, which rehydrates from it — but no test today distinguishes a wrong permutation of them, so a mis-ordered merge would be caught by review or not at all. **Every wave from 2 onward adds its own build-order guard to `test/guards/buildorder_test.go` before its branches merge**, written and observed failing against the wrong order first, exactly as `TestGuard_Phase0BeforeStore` was; the wave's verification checkpoint names that guard. A wave that merges without one has an unverified order and must say so in its checkpoint report.
