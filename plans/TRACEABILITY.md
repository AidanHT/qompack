...
This matrix proves the subplan set preserves the entire design document. Every gap, phase, layer, tool, revision item, and closing-note priority in `Qompack.md` maps to exactly one **owning** subplan (others consume its output). Ownership below is taken from each subplan's own header metadata (`**Gaps closed:**` / `**Design sections:**`) and mirrors the §9 gap-traceability matrix of the design document. Re-testing ownership is listed under Verification.
...
| G1.5 | No task-boundary signals | SP-08 | Todo/git/test signals fed to the scheduler |
...
| G3.2 | Pointerless tombstones | SP-08 | Addressable tombstones |
...
| 2 — Negative knowledge | Bloom, descriptors, staleness (GA), MCP `record_eliminated`/`already_tried`, CMS/HLL | SP-03 (primitives) + SP-09 (closes) + SP-13 (tools) | V3, V4 |
...
| 7 — Refinement | O4 warm start, demand-driven promotion, per-segment Blooms, ski-rental, progressive truncation | SP-16 | V5, V6 |
...
| Surface | Owner |
|---|---|
| `PostToolUse`, `UserPromptSubmit`, `Stop`/`SubagentStop`, `SessionEnd` | SP-08 (entry points on SP-05's substrate) |
...
| O2 | Per-tool canonicalization + MinHash near-dedup | SP-04 |
