...
| 9 | `assignment_secret` | `(?i)\b(?:(?:password\|passwd\|secret\|api[_-]?key\|access[_-]?key\|client[_-]?secret\|auth[_-]?token\|access[_-]?token\|refresh[_-]?token\|api[_-]?token)\b[ \t]*[:=]\|token\b[ \t]*=)[ \t]*("[^"\n]{4,}"\|'[^'\n]{4,}'\|[^\s,;"'\n]{1,})` | 1 |
...
> **V2 reconciliation — rule 9 as printed above IS the shipped pattern, and three things about it are forced.** Recorded as **D3** and **D4** in *Spec resolutions* below.
>
> - The **unquoted-value quantifier is `{1,}`, not `{4,}`.** `redacttest`'s frozen bounded-growth fixture is `password=x` — a one-byte value — followed by `require.NotEmpty(t, matches, "fixture sanity: the embedded password= assignment must match")`. A `{4,}` unquoted alternative cannot match it at all, so the draft's pattern fails the frozen suite regardless of where the `minMatchBytes` floor is applied. The quoted alternatives keep `{4,}`: a quoted value carries its own delimiters, so the short-value case only reaches the unquoted arm.
> - **Bare `token` is accepted only with `=`.** The `:` form is limited to the unambiguous key names, so `redacttest`'s `github_token` **negative** fixture `token: ghz_1234…` stays unmatched (D3).
> - The key list gained `auth[_-]?token`, `access[_-]?token`, `refresh[_-]?token` and `api[_-]?token` alongside the original names.
>
> Each shipped rule additionally sits behind a mandatory-literal `bytes.Contains` prefilter (D19); rule 9's literal set is `password`, `passwd`, `secret`, `key`, `token`, which every alternative provably contains under `(?i)`. Copy the pattern from this table verbatim — a subplan that "restores" `{4,}` breaks the frozen conformance suite on the first run.
...
>
> - **The per-object `Sync()` before the rename is gone.** One 100 KB result is tens to hundreds of chunks, and serialized fsyncs measured **1.9 s** against a 3 ms B-C budget. The rename still supplies **atomicity** — no torn object is ever visible — and **durability is batched into `Flush`**, which is how borg, restic and git commit a repository transaction (git does not fsync loose objects by default either). **A crash can now lose an object but can never corrupt one.** The reachable crash states are: neither object nor root line (clean); an object with no root line (an orphan, which GC collects); or a root line without its object, which `GetChunk`/`Open` report as `core.ErrNotFound` and `Has` catches by falling back to a stat. That last state is degraded-**but-detected**, and detection is the part that ships here: `Has` falls back to `objectExists`, and `GetChunk`/`getObject` report `core.ErrNotFound` rather than returning short or wrong bytes.
>
>   **Where the residual is discharged (decided, do not re-file as open).** `qompack fsck` check 3 (`roots`) already reports every root line whose `ChunkRef.Hash` fails `store.Has`. SP-17 gives that check a real `--repair` action rather than leaving it report-only: for a root whose chunks are **unrecoverable**, `--repair` appends a `{"v":1,"op":"gc","root":"<hash>","ts":<unixMilli>}` retirement tombstone to `index/roots.jsonl` — the store's own retirement record, written by its own append-only writer, so the repair adds a line and never rewrites or deletes one — and records the affected `tool_use` ids in the fsck report as the set that must be **re-observed**. Deleting the root line was never an option (it would destroy the index); retiring it is the operation the store already performs for a root it no longer wants indexed, and re-observation is what actually restores the content. That is where this residual is discharged, and SP-17's check-3 row is the place it is written down.
...
| D4 | The `if e-s < 3 { continue }` growth-bound floor drops `redacttest`'s bounded-growth fixture `password=x`, whose value span is one byte, but that suite asserts a match **fires**. | Two changes, both forced by the same one-byte fixture. **(a)** The floor is applied to the **full match span**, not to the replaced group span. Density — and therefore the growth bound — is governed by the full span (per-rule matches never overlap), so the bound is preserved while a short secret value is still redacted. The ≥3-byte **admission** requirement for user-supplied patterns is unchanged, which is where the adversarial growth risk actually lives. **(b)** Rule 9's unquoted-value quantifier was relaxed from `{4,}` to **`{1,}`**. Moving the floor is necessary but not sufficient: `[^\s,;"'\n]{4,}` cannot match `x` in the first place, so the regex never produces a match for the floor to keep. The frozen fixture is `b.WriteString("password=x\n")` in `internal/redact/redacttest/behaviour.go`, asserted with `require.NotEmpty` on the very next lines, so this is a property of the suite the merge gate runs, not a preference. The quoted alternatives keep `{4,}`. The rule table above prints the shipped pattern. |
...
| **D15** | **`putObject`'s per-object `Sync()` before the rename is incompatible with its own budget.** | Measured: one 100 KB result is tens to hundreds of chunks, and serialized fsyncs cost **1.9 s** against a 3 ms budget (B-C). Removed. The rename still provides **atomicity** — no torn object is ever visible — and **durability** is batched into `Flush`, which is exactly how borg, restic and git commit a repository transaction (git does not fsync loose objects by default either). A crash can now lose an object but can never corrupt one. Crash states are: neither object nor root line (clean); object without a root line (an orphan, which GC collects); or a root line without its object, which `GetChunk`/`Open` report as `core.ErrNotFound` and `Has` catches by falling back to a stat. That last state is degraded-but-detected rather than silent corruption: detection ships here (`Has` falls back to `objectExists`; `GetChunk`/`getObject` report `core.ErrNotFound`). The residual is discharged by **SP-17's `fsck` check 3 (`roots`)**, which detects it already and gains a real `--repair` action: for a root whose chunks are unrecoverable, `--repair` **appends** a `{"v":1,"op":"gc","root":…,"ts":…}` retirement tombstone — the store's own retirement mechanism, so the repair is append-only and rewrites nothing — and records the affected `tool_use` ids in the report as the set that must be **re-observed**. Deleting the root line stays forbidden; retirement plus re-observation is the repair. |
