...
func (r *rx) Redact(in []byte) ([]byte, []Match) {
    ...
    for _, rule := range r.rules {
        for _, loc := range rule.re.FindAllSubmatchIndex(in, -1) {
            ...
            // A non-PEM rule may never swallow the entire input (§5.22a fuzz requirement).
            if rule.name != "pem_private_key" && s == 0 && e == len(in) { continue }
    ...
    for _, h := range hits {
        ...
        out = append(out, "«redacted:"...); out = append(out, h.Rule...); out = append(out, '»')
        prev = h.Offset + h.Len
    }
...
§5.22a's "no rule ever matches across the whole input" is asserted for the nine non-PEM rules only. `pem_private_key` is the single documented exemption: a tool result that *is* nothing but a private key must still be fully redacted, and refusing the match there would put the secret in `objects/` — the exact outcome §13 invariant 7 forbids. **Failure modes:** an invalid or inadmissible user pattern is logged once through `logging.Loud` at `New` and skipped, never fatal; `Redact` never returns an error and never returns a nil slice for a non-empty input.
